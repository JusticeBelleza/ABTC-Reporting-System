...
function DashboardContent() {
  ...
  const availableMonths = useMemo(() => (
    year === currentRealYear ? MONTHS.slice(0, currentDate.getMonth() + 1) : MONTHS
  ), [year, currentRealYear]);

  // View & UI State
  const [activeNav, setActiveNav] = useState('dashboard'); // Controls Dashboard vs Reports tab
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 relative">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 no-print transition-all duration-300 shadow-sm shrink-0">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between relative">
          ...
          {/* --- Desktop Navigation for ALL Users --- */}
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-3 w-1/3">
            ...
            <div 
              ...
              className="flex items-center gap-2 sm:gap-2.5 pl-2 sm:pl-3 pr-1 py-1 bg-white border border-slate-200 rounded-full shadow-sm hover:border-slate-400 active:scale-95 transition-all duration-300 cursor-pointer group"
            >
            ...
            <div className="flex items-center bg-slate-100 p-1 rounded-full border border-slate-200 shadow-inner">
              <button onClick={() => setShowSettingsModal(true)} className="p-1.5 text-slate-500 hover:text-slate-900 hover:bg-white rounded-full transition-all active:scale-90" title="System Settings">
                <Settings size={16} className="sm:w-[18px] sm:h-[18px]" strokeWidth={2.5} />
              </button>
              <div className="w-px h-3 sm:h-4 bg-slate-300 mx-0.5"></div>
              <button onClick={() => setShowLogoutModal(true)} className="p-1.5 text-slate-500 hover:text-rose-600 hover:bg-white rounded-full transition-all active:scale-90" title="Log Out">
      ...
      <main className="flex-1 p-4 md:p-8" id="report-content">
        {/* Conditional Rendering for Analytics Tab */}
        {activeNav === 'reports' ? (
           <AnalyticsOverview 
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4 animate-in fade-in duration-200">
                <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-2xl max-w-sm w-full animate-in zoom-in-95 duration-200">
                    <div className="flex flex-col items-center text-center">
                        ...
                        <p className="text-sm text-slate-500 mt-2 mb-6 leading-relaxed">Are you sure you want to log out of your account? You will need to sign in again to access the system.</p>
                        <div className="flex flex-col sm:flex-row gap-3 w-full">
          ...
          <div className="bg-slate-900/95 text-slate-400 px-5 sm:px-8 py-3 rounded-2xl sm:rounded-full shadow-xl border border-slate-800 flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full sm:w-fit transition-all hover:border-slate-700">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black whitespace-nowrap tracking-wider text-slate-300">
            ...
            <a 
              href="https://github.com/JusticeBelleza" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="p-2 bg-slate-800 rounded-xl text-yellow-400 hover:text-white hover:bg-slate-700 active:scale-90 transition-all shadow-inner group"
              title="Github Profile"
            >
              <Github size={14} strokeWidth={2.5} className="group-hover:rotate-12 transition-transform"/>
            </a>
          </div>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-bottom-10 duration-500 fade-in w-max no-print">
        <div className="flex items-center bg-white/90 backdrop-blur-xl p-1.5 rounded-full border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-5 py-3 rounded-full text-sm font-bold transition-all w-[130px] ${
              activeNav === 'dashboard' 
          ...
          <button
            onClick={() => { setActiveNav('reports'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
            className={`flex items-center justify-center gap-2 px-5 py-3 rounded-full text-sm font-bold transition-all w-[130px] ${
              activeNav === 'reports' 
                ? 'bg-slate-900 text-white shadow-md' 
                : 'text-slate-500 hover:text-slate-900 active:bg-slate-100'
